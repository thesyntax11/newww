...
export default function MemoryPanel({ sessionId, open, onClose }: MemoryPanelProps) {
  ...
  async function handleAdd() {
    ...
    setAdding(true);
    try { await fetch("/api/memory", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, content: newContent, category: newCategory, importance: newImportance }) }); setNewContent(""); setNewImportance(5); setNewCategory("general"); load(); } finally { setAdding(false); }
  }
              ...
              : memories.map((m) => (
                <div key={m.id} className="group flex items-start gap-2 rounded-lg border border-line bg-panel-soft/50 p-3">
                  <div className="flex-1">
                    <div className="mb-1 flex items-center gap-2"><span className="rounded-full bg-plasma/15 px-2 py-0.5 font-mono text-[10px] text-plasma-soft">{m.category}</span><span className="font-mono text-[10px] text-mist">öncelik {m.importance}</span></div>
